...
TEST_F(DESIRED_STATE_NODE_TEST_SUITE, test_integrated_desired_state_node_subscription)
{
    std::shared_ptr<DesiredStateNode> desired_node_ptr = std::make_shared<DesiredStateNode>();
    
    Interface::node_t temp_node = rclcpp::Node::make_shared("change_desired_state_node_client");
    Interface::change_desired_state_client_t change_desired_state_node_client =  temp_node->create_client<scion_types::srv::ChangeDesiredState>("change_desired_state");
    ...
    Interface::node_t temp_node2 = rclcpp::Node::make_shared("get_desired_state_node_client");
    Interface::get_desired_state_client_t get_desired_state_node_client =  temp_node2->create_client<scion_types::srv::GetDesiredState>("get_desired_state");
    
    auto get_desired_state_request = std::make_shared<scion_types::srv::GetDesiredState::Request>();
    get_desired_state_request->requester_name = "Zix & Friends";
    auto future2 = get_desired_state_node_client->async_send_request(get_desired_state_request);
    
    rclcpp::spin_some(desired_node_ptr);
    rclcpp::spin_some(temp_node2);
    rclcpp::spin_some(desired_node_ptr);
    rclcpp::spin_some(temp_node2);
    rclcpp::spin_some(desired_node_ptr);
    rclcpp::spin_some(temp_node2);
