 ...
 */
Controller::Controller(const Robot& robot) : Component("controller"), robot{robot}
{
...
void Controller::processStickInputs(std::vector<float>& ctrl_vals)
{
    ...
    std::vector<float> thrust_vals = robot.getThrustMapper() * ctrl_vals;
    robot.getCanClient()->make_motor_request(thrust_vals, robot.getMotorCount(), MAX_POWER);
}
